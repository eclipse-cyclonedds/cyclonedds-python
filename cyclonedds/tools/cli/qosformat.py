...
def fmtime(nanoseconds: int) -> str:
    ...
    ret += "" if milliseconds == 0 else f"{milliseconds} millisecond{'s' if milliseconds > 0 else ''}, "
    ret += "" if microseconds == 0 else f"{microseconds} microsecond{'s' if microseconds > 0 else ''}, "
    ret += "" if nanoseconds == 0 else f"{nanoseconds} nanosecond{'s' if nanoseconds > 0 else ''}, "
...
def rich_format_policy(policy: Policy):
    if policy.__scope__ in ['Durability', 'Ownership', 'DestinationOrder', 'IgnoreLocal'] or policy in [Policy.Reliability.BestEffort, Policy.History.KeepAll]:
        return f"[bold bright_magenta]{policy.__class__.__name__}[/]"
    if isinstance(policy, Policy.Reliability.Reliable):
        return Pretty(Policy.Reliability.Reliable(max_blocking_time=fmtime(policy.max_blocking_time)))
    ...
    if isinstance(policy, Policy.ReaderDataLifecycle):
        return Pretty(Policy.ReaderDataLifecycle(
            autopurge_nowriter_samples_delay=fmtime(policy.autopurge_nowriter_samples_delay),
            autopurge_disposed_samples_delay=fmtime(policy.autopurge_disposed_samples_delay))
        )
